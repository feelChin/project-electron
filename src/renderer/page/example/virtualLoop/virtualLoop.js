...
export const MyScroll = class MyCustomScroll {
	constructor({
		element,
		barHeight = 20,
		onScroll = () => {},
		...
		this.wheelConfig = {
			...
			barScrollPoint:
				100 - (barHeight / this.root.parentNode.offsetHeight) * 100,
			drag: false, // 是否拖拽
			showbarTimer: null,
		};
	...
	update() {
		this.wheelConfig = {
			...
			barScrollPoint:
				100 - (this.barHeight / this.root.parentNode.offsetHeight) * 100,
		};
	...
	createBarElement() {
		...
		section.style.setProperty("--barHeight", this.barHeight + "px");
		...
		this.root.addEventListener(
			"pointerdown",
			this.disableOnPointerDown.bind(this)
		);
	...
	handlePointerDown(e) {
		...
		this.el_bar.classList.add("drag");

		const { count, childHeight } = this.wheelConfig;
	...
	handlePointerMove(e) {
		if (this.wheelConfig.drag) {
			...
			const { myScrollHeight, barScrollPoint, childHeight } = this.wheelConfig;

			const allScroll = (barScrollPoint * myScrollHeight) / 100;

			const point =
				(e.clientY - this.startPoint) / allScroll + this.startPointScroll;
	...
	handlePointerUp() {
		if (this.wheelConfig.drag) {
			...
			this.el_bar.classList.remove("drag");
		}
	}
...
export default class MyVirtualList {
	constructor({
		element,
		result = [],
		height = 50,
	...
	createBarElement() {
		...
		this.myCustomScroll = new MyScroll({
			element: this.realNodeName + " .virtual",
