export default class Clip {
	...
	point_mouseStart(e) {
		...
		const { width, height } = getComputedStyle(target.parentNode);
		let { x, y } = this.offset;
		...
		this.frame.classList.contains("active") &&
	...
	// 移动鼠标
	mouseMove(e) {
		if (e.target.className.indexOf("app") === -1) return;

		if (this.lock) {
			this.createElement();

			const { x, y } = this.offset;
			const { app_w, app_h } = this;
			const { offsetX, offsetY } = e;

			// 得到位移方位
			const { position, str, key } = this.judgePostion(this.offset, e);
			...
			this.rafId = requestAnimationFrame(() => {
				...
				if (!this.edit) {
					this.app.style.setProperty(
						"--line_width",
						diagonal.toFixed(2) + "px"
					);
					this.app.style.setProperty(
						"--line_deg",
						angleInDegrees.toFixed(2) * ([1, 4].includes(key) ? 1 : -1) + "deg"
					);
					this.app.style.setProperty(
						"--line_top",
						[1, 4].includes(key) ? 0 : 100 + "%"
					);
	...
	frame_mouseStart(e) {
		console.log(e);
		if (e.target.className.indexOf("frame") === -1) return;

		this.frameLock = true;

		const { offsetTop, offsetLeft } = this.frame;
		const { offsetX, offsetY } = e;
	...
	frame_mouseMove(e) {
		if (this.frameLock) {
			const { offsetX, offsetY } = e;
			const { app_w, app_h } = this;
			const { w: frame_w, h: frame_h } = this.offset;

			const { x, y, offsetLeft, offsetTop } = this.frameOffset;
			...
			// 得到位移方位
			const { position } = this.judgePostion(this.frameOffset, e);
	...
	renderPoint(result = "") {
		Object.keys(this.locationConfig).forEach((item) => {
			...
			const { key, transform } = this.locationConfig[item];
			const [trs_x, trs_y] = transform;

			result += `<div class="point" key="${key}" style="${pos_x}:0;${pos_y}:0;transform:translate(calc(${trs_x} * var(--point) / 2), calc(${trs_y} * var(--point) / 2))"></div>`;
		});
	...
	// 判断裁剪方向
	judgePostion(originOffset, nowOffset, result = []) {
		const { x, y } = originOffset;
		const { offsetX, offsetY } = nowOffset;

		result[0] = offsetY > y ? "bottom" : "top";
		result[1] = offsetX > x ? "right" : "left";

		const str = result.join(",");
		const { info, key } = this.locationConfig[str];
	...
	onWatch() {
		...
		const { offsetTop, offsetLeft } = this.frame;
		const { width, height } = getComputedStyle(this.frame);
