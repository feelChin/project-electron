...
contextBridge.exposeInMainWorld("electron", {
  ...
  // 打开浏览器
  openBrowser: (val) => {
    function isDomainName(val2) {
      const domainRegex = /^(?:(?:https?|ftp):\/\/)?(?:www\.)?([a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*\.[a-zA-Z]{2,})$/;
      return domainRegex.test(val2);
    }
    function containsHttpOrHttps(val2) {
      const httpRegex = /(http|https)/;
      return httpRegex.test(val2);
    }
    ipcRenderer.send(
      "open-browser",
      isDomainName(val) ? containsHttpOrHttps(val) ? val : `http://${val}` : `https://cn.bing.com/search?q=${val}`
    );
  },
  // 下载全量安装包
  windowDownload: () => {
